...
import { updateDoctor } from "../../features/Doctors/DoctorSlices";
...
function DetailDoctor(props) {
  ...
  const doctor = useSelector((state) => state.doctor.show);

  useEffect(() => {
    if (doctor) {
      setUid(doctor.id);
      setName(doctor.attributes.name);
      setGender({ name: doctor.attributes.gender });
      setAddress(doctor.attributes.address);
      setPhone(doctor.attributes.phone);
      setSpecialist(doctor.attributes.specialist);
    }
  }, [doctor]);
    ...
    <div className="flex flex-row m-6 mt-4">
      <Card className="w-full">
        <div className="text-xl font-bold uppercase border-b border-sky-500 flex justify-between">
          <div className="m-6 text-gray-700">Add Doctor</div>
